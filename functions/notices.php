...
if ( ! class_exists( 'WOOKIT_Admin_Notices' ) ) {

    class WOOKIT_Admin_Notices {
        ...
        public function action_admin_init() {
            $dismiss_option = filter_input( INPUT_GET, 'wookit_dismiss', FILTER_SANITIZE_STRING );
            if ( is_string( $dismiss_option ) ) {
                update_option( "wookit_dismissed_$dismiss_option", true );
                wp_die();
            }
        }
            ...
            foreach ( explode( ',', self::TYPES ) as $type ) {
                foreach ( $this->admin_notices->{$type} as $admin_notice ) {
                    ...
                    $dismiss_url = add_query_arg( array(
                        'wookit_dismiss' => $admin_notice->dismiss_option
                    ), admin_url() );

                    if ( ! get_option( "wookit_dismissed_{$admin_notice->dismiss_option}" ) ) {
                        ?><div
                            class="notice wookit-notice notice-<?php echo $type;

                            if ( $admin_notice->dismiss_option ) {
                                echo ' is-dismissible" data-dismiss-url="' . esc_url( $dismiss_url );
                            } ?>">

                            <h2><?php echo "WooKit $type"; ?></h2>
...
WOOKIT_Admin_Notices::get_instance();
